...
import os,sys,re,time,shutil,pymysql
...
class download1:
    ...
    def getHTMLText1(self,url,webID):
        try:
            ...
            if number > 100:
                print(webID,"图片过多\n")
                self.rs.append(webID[2:])  #记录图片数量太多的url，下次不再比较
                return "",number
            if (webID[2:] in self.rs):
                print(webID[2:],"已经爬取\n")
                return "",number
    ...
    def getInfo(self,webID):
        ...
        try:
            ...
            if os.path.exists(ospath):
                print(webID,"文件已经存在\n")
                shutil.rmtree(ospath)      #数据库没有 文件夹里有 认为爬取不完整 将会删除此文件
                return
            else:
                os.makedirs(ospath)
            ...
            for i in Info:
                if i.name == 'p':
                    ...
                    for j in i:
                        if j.name == 'a':
                            lis.append(j.string)  #把a标签的文字添加到文本
                            continue
                        if j.name == 'img':   #忽略文本的小图片 比如一些小表情
                            continue
                        else:
                            lis.append(str(j))
                    lis.append("</p>")
            ...
            print(webID,"爬取完毕")
            self.number +=1
            self.size += self.get_FileSize(ospath)
            return
        except:
            return
    ...
    def main(self):
        ...
        self.lb.insert(END, '时间:'+str(self.endtime-self.begintime) +'s,爬取了  '+str(self.number) + '篇游记' + '，文件大小：' + str(self.size))
        self.root.update()
