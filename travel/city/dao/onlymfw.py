...
def getInfo(webID):
    ...
    #获得每个URL的文件对象
    html,number= getHTMLText1(url,webID)
    ...
    while page:
        ...
        for i in url_list:
            if i[2:] in rs:
                ...
                continue
            print(i[2:])
            pool.apply_async(getInfo,args=(i,))  # 传入进程函数名和url列表 产生子进程
        pool.close()  # 关闭进程池
        #pool.join()  # 主进程阻塞
...
root.geometry('600x420+300+200')  # 设置窗口大小 注意：是x 不是*
...
l = Label(root, font=("Arial", 12), width=8, height=3)
l.pack()

l.config(text='正在爬取...')
button = Button(root, text='开始爬取', font=('微软雅黑', 10), command=main)
